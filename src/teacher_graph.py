...
class GraphConvolution(nn.Module):
    ...
    def forward(self, feat, adj):
        ...
        if getattr(self, 'efficient_query_only', False):
            # Only node zero is returned below. Associate its aggregation before
            # the feature projection, avoiding unused node projections/outputs.
            query = torch.bmm(adj[:, :1, :], x).squeeze(1)
            output = torch.matmul(query, self.gcn_weights)
            if self.bias:
                output = output + self.gcn_bias[0]
